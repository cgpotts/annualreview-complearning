...
familiar CYK parsing algorithm (in Grammar.gen). Thus, the structures
are like this:
    ...
    def allcombos(self, c1, c2):
        ...
        for left, right, mother, app_order in self.rules:
            if left == c1[0] and right == c2[0]:
                sem = [c1[1], c2[1]]
                results.append((mother, "%s(%s)" % (sem[app_order[0]], sem[app_order[1]])))
        return results

    def sem(self, lf):
        """Interpret, as Python code, the root of a logical form generated by this grammar."""
